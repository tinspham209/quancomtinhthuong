...
export function useGetRestaurantById(
  ...
  const handleGet: QueryFunction<Restaurant> = () => {
    return responseWrapper<Restaurant>(apiClient.getRestaurantById, [options?.restaurantId]);
  };
  const {
    data,
    error,
...
export function useUpdateRestaurant(
  ...
  const { mutate: updateRestaurant, isLoading } = useMutation<
    ...
    CreateRestaurantPayload & { restaurantId: string }
  >({
    mutationFn: async (payload: CreateRestaurantPayload & { restaurantId: string }) => {
...
export function useDeleteRestaurant(
  ...
  >({
    mutationFn: async (payload: { restaurantId: string }) => {
      return responseWrapper(apiClient.deleteRestaurantById, [payload.restaurantId]);
    },
    ...options,
  });
