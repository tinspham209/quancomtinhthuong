...
export function useGetDishesByRestaurantId(
  ...
  const handleGet: QueryFunction<Dish[]> = () => {
    return responseWrapper<Dish[]>(apiClient.getDishesByRestaurantId, [options?.restaurantId]);
  };
  const {
    data,
    error,
  ...
  } = useQuery<Dish[], Error>([`/dishes`, { restaurantId: options?.restaurantId }], {
    queryFn: handleGet,
...
export function useCreateDishes(options?: UseMutationOptions<Dish[], Error, CreateDishPayload>) {
  const { mutate: createDishes, isLoading } = useMutation<Dish[], Error, CreateDishPayload>({
